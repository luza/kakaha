			...
			<table class="post_table">
				<tr>
					<td colspan="2" class="row_title">
						...
						<div><a style="text-decoration: none; color: #AAA;" class="script_a" href="?thread=<?php echo $thread['id']; ?>&insert=<?php echo $thread['id']; ?>#postbox">#<?php echo $thread['id']; ?></a>&nbsp;&nbsp;<span class="subject"><?php echo $thread['subject'] ? $thread['subject'] . "&nbsp;&nbsp;" : NULL; ?></span><?php echo date($lang['DATE_FORMAT'], $thread['post_time']); ?>&nbsp;@&nbsp;<?php echo $thread['name'] ? $thread['name'] : $lang['ANONYMOUS']; ?><?php if ($admin_mode) echo " ({$thread['ip']})"; ?>&nbsp;[<a href="?thread=<?php echo $thread['id']; ?>"><?php echo $lang['RESPONSE']; ?></a>]</div>
					</td>
				</tr>
...
<?php if ($thread['posts_count'] > 5) { ?>
		<div class="separator"><?php printf($lang['POSTS_SKIPPED'], $thread['posts_count'] - 5, "<a href=\"?thread=" . $thread['id'] . "\">" . $lang['RESPONSE'] . "</a>"); ?></div>
<?php } ?>
				...
				<table class="post_table">
					<tr>
						<td colspan="2" class="row_title">
							...
							<div><a style="text-decoration: none; color: #AAA;" class="script_a" href="?thread=<?php echo $thread['id']; ?>&insert=<?php echo $post['id']; ?>#postbox">#<?php echo $post['id']; ?></a>&nbsp;&nbsp;<span class="subject"><?php echo $post['subject'] ? $post['subject'] . "&nbsp;&nbsp;" : NULL; ?></span><?php echo date($lang['DATE_FORMAT'], $post['post_time']); ?>&nbsp;@&nbsp;<?php echo $post['name'] ? $post['name'] : $lang['ANONYMOUS']; ?><?php if ($admin_mode) echo " ({$post['ip']})"; ?>
						</td>
					</tr>
					<tr class="row_message">
<?php if ($post['image']) { ?>
...
<?php for ($i=0; $i<$pages_count; $i++) { ?>
	<?php if ($i == $page) { ?>
		<?php echo $i+1; ?>
	<?php } else { ?>
		<a href="<?php echo $base_uri; ?>?page=<?php echo $i+1; ?>"><?php echo $i+1; ?></a>
	<?php } ?>
<?php } ?>
]</div>
